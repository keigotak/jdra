...
def split_to_dev_fold(document_ids, num_fold, index_fold=0):
    ...
    return train_set, dev_set, test_setgen

def get_properties(mode):
    if mode == 'rinna-gpt2':
        return 'rinna/japanese-gpt2-medium', './results/rinna-japanese-gpt2-medium.gen', 100
    elif mode == 'tohoku-bert':
        return 'cl-tohoku/bert-base-japanese-whole-word-masking', './results/bert-base-japanese-whole-word-masking.gen', 100
    elif mode == 'mbart':
        return 'facebook/mbart-large-cc25', './results/mbart-large-cc25.gen', 100
    elif mode == 't5-base-encoder':
        return 'megagonlabs/t5-base-japanese-web', './results/t5-base-japanese-web.gen', 100
    elif mode == 't5-base-decoder':
        return 'megagonlabs/t5-base-japanese-web', './results/t5-base-japanese-web.gen', 100
    elif mode =='rinna-roberta':
        return 'rinna/japanese-roberta-base', './results/rinna-japanese-roberta-base.gen', 100
    elif mode == 'nlp-waseda-roberta-base-japanese':
        return 'nlp-waseda/roberta-base-japanese', './results/nlp-waseda-roberta-base-japanese.gen', 100
    elif mode == 'nlp-waseda-roberta-large-japanese':
        return 'nlp-waseda/roberta-large-japanese', './results/nlp-waseda-roberta-large-japanese.gen', 100
    elif mode == 'nlp-waseda-roberta-base-japanese-with-auto-jumanpp':
        return 'nlp-waseda/roberta-base-japanese-with-auto-jumanpp', './results/nlp-waseda-roberta-base-japanese.gen', 100
    elif mode == 'nlp-waseda-roberta-large-japanese-with-auto-jumanpp':
        return 'nlp-waseda/roberta-large-japanese-with-auto-jumanpp', './results/nlp-waseda-roberta-large-japanese.gen', 100
    elif mode == 'rinna-japanese-gpt-1b':
        return 'rinna/japanese-gpt-1b', './results/rinna-japanese-gpt-1b.gen', 100
    elif mode == 'xlm-roberta-large':
        return 'xlm-roberta-large', './results/xlm-roberta-large.gen', 100
    elif mode == 'xlm-roberta-base':
        return 'xlm-roberta-base', './results/xlm-roberta-base.gen', 100
...
def train_model(run_mode='rinna-gpt2', index_fold=0):
    ...
    DEVICE = 'cuda:0'
    DATASET_MODE = 'expert.230205' # crowdsourcing, expert, only_expert, with_crowdsourcing, expert.230205, crowdsourcing.230205
    with_print_logits = False

    model_name, OUTPUT_PATH, _ = get_properties(run_mode)
    OUTPUT_PATH = OUTPUT_PATH + f'.230302.{seed}'
    ...
    with_load_weight = True
    if with_load_weight:
        # model = AutoModelForCausalLM.from_pretrained('./results/xlm-roberta-large.pt.230301')
        model = GPT2LMHeadModel.from_pretrained('./results/xlm-roberta-large.pt.230301')
        tokenizer = AutoTokenizer.from_pretrained('./results/xlm-roberta-large.pt.230301')
    ...
    # token_ans = '[ANS]'
    # tokenizer.add_special_tokens({'additional_special_tokens': [token_ans]})
    # model.resize_token_embeddings(len(tokenizer))
    # id_ans = tokenizer._convert_token_to_id_with_added_voc(token_ans)

    header = '問い：文１と文２の関係を 原因・理由，目的，条件，根拠，対比，逆接・譲歩，その他根拠，談話関係なし の中から選びなさい．'
    s1, y = [tokenizer(f"{header}\n\n文１：{d['s1']}\n文２：{d['s2']}\n\n答え：{d['reason']}", return_tensors='pt') for d in datasets['train']], [d['reason'] for d in datasets['train']]
    train_dataset = Dataset(s1, y, datasets['train'])
    s1, y = [tokenizer(f"{header}\n\n文１：{d['s1']}\n文２：{d['s2']}\n\n答え：", return_tensors='pt') for d in datasets['dev']], [d['reason'] for d in datasets['dev']]
    dev_dataset = Dataset(s1, y, datasets['dev'])
    s1, y = [tokenizer(f"{header}\n\n文１：{d['s1']}\n文２：{d['s2']}\n\n答え：", return_tensors='pt') for d in datasets['test']], [d['reason'] for d in datasets['test']]
    ...
    for e in range(EPOCHS):
        ...
        for i, (s1, y , m1, allx) in enumerate(train_dataloader):
            s1, m1 = s1.to(DEVICE), m1.to(DEVICE)
            logits = model(s1).logits

            loss = loss_func(logits, s1).mean()
        ...
        with torch.inference_mode():
            ...
            dev_total_loss = []
            dev_tt, dev_ff = 0, 0
            total_dev_y, total_dev_pred, total_dev_allx = [], [], []
            dev_predictions = []
            for i, (s1, y , m1, allx) in enumerate(dev_dataloader):
                ...
                for s, label, m in zip(s1, y , m1):
                    cur_ids = s[m == 1][:-1].unsqueeze(0)
                    initial_length = cur_ids.shape[1]
                    ...
                    predicted_label = tokenizer.decode(cur_ids.squeeze()[initial_length:]).strip()
                    total_dev_y.append(label)
                    total_dev_pred.append(predicted_label)
                    if predicted_label.strip() == label:
                        dev_tt += 1
                    else:
                        dev_ff += 1
                total_dev_allx.extend(allx)

            dev_predictions = [items[2] | {'predicted_label': pred, 'label': label} for items, pred, label in zip(total_dev_allx, total_dev_pred, total_dev_y)]

            if with_print_logits:
                print(f'{logits.item()}, {predicted_label}, {y}')

            dev_rets = get_metrics_scores([label_indexer[i] for i in total_dev_y], [label_indexer[i] if i in label_indexer else len(label_indexer) for i in total_dev_pred], label_indexer)
            ...
            test_total_loss = []
            test_tt, test_ff = 0, 0
            total_test_y, total_test_pred, total_test_allx = [], [], []
            test_predictions = []
            for i, (s1, y , m1, allx) in enumerate(test_dataloader):
                ...
                for s, label, m in zip(s1, y , m1):
                    ...
                    for _ in range(MAX_NEW_TOKENS):
                        ...
                        cur_ids = torch.cat([cur_ids, next_token_id], dim=1)
                    predicted_label = tokenizer.decode(cur_ids.squeeze()[initial_length:]).strip()
                    total_test_y.append(label)
                    total_test_pred.append(predicted_label)
                    if predicted_label.strip() == label:
                        test_tt += 1
                    else:
                        test_ff += 1
                total_test_allx.extend(allx)

            test_predictions = [items[2] | {'predicted_label': pred, 'label': label} for items, pred, label in zip(total_test_allx, total_test_pred, total_test_y)]

            if with_print_logits:
                print(f'{logits.item()}, {predicted_label}, {y}')

            test_rets = get_metrics_scores([label_indexer[i] for i in total_test_y], [label_indexer[i] if i in label_indexer else len(label_indexer) for i in total_test_pred], label_indexer)
...
if __name__ == '__main__':
    ...
    run_modes = ['rinna-japanese-gpt-1b']
    if is_single:
