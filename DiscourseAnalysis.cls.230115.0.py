...
def tokenize_sentences(dataset, tokenizer):
    ...
    for items in dataset:
        tokens1 = tokenizer(items['s1'], text_pair=items['s2'], return_tensors='pt')
        _dataset.append(items | {'token1': tokens1, 'pad_token_id': tokenizer.pad_token_id})
    return _dataset
...
def train_model(run_mode='rinna-gpt2'):
    ...
    for e in range(EPOCHS):
        ...
        if accelerator is None:
            ...
            with Path(f'{OUTPUT_PATH}/result.cls.{model_name.replace("/", ".")}.csv').open('w') as f:
