...
class Classifier(nn.Module):
    def __init__(self, hidden_size, num_class, pooling_method='mean'):
    ...
    def forward(self, x1, mask1):
        ...
        if self.pooling_method == 'mean':
            x1 = self.pooling_mean(x1, mask1)
        elif self.pooling_method == 'max':
            x1 = self.pooling_max(x1, mask1)
...
def get_properties(mode):
    if mode == 'rinna-gpt2':
        return 'rinna/japanese-gpt2-medium', './results/rinna-japanese-gpt2-medium.gconcat', 100
    elif mode == 'tohoku-bert':
        return 'cl-tohoku/bert-base-japanese-whole-word-masking', './results/bert-base-japanese-whole-word-masking.gconcat', 100
    elif mode == 'mbart':
        return 'facebook/mbart-large-cc25', './results/mbart-large-cc25.gconcat', 100
    elif mode == 't5-base-encoder':
        return 'megagonlabs/t5-base-japanese-web', './results/t5-base-japanese-web.gconcat', 100
    elif mode == 't5-base-decoder':
        return 'megagonlabs/t5-base-japanese-web', './results/t5-base-japanese-web.gconcat', 100
    elif mode =='rinna-roberta':
        return 'rinna/japanese-roberta-base', './results/rinna-japanese-roberta-base.gconcat', 100
    elif mode == 'nlp-waseda-roberta-base-japanese':
        return 'nlp-waseda/roberta-base-japanese', './results/nlp-waseda-roberta-base-japanese.gconcat', 100
    elif mode == 'nlp-waseda-roberta-large-japanese':
        return 'nlp-waseda/roberta-large-japanese', './results/nlp-waseda-roberta-large-japanese.gconcat', 100
    elif mode == 'nlp-waseda-roberta-base-japanese-with-auto-jumanpp':
        return 'nlp-waseda/roberta-base-japanese-with-auto-jumanpp', './results/nlp-waseda-roberta-base-japanese.gconcat', 100
    elif mode == 'nlp-waseda-roberta-large-japanese-with-auto-jumanpp':
        return 'nlp-waseda/roberta-large-japanese-with-auto-jumanpp', './results/nlp-waseda-roberta-large-japanese.gconcat', 100
    elif mode == 'rinna-japanese-gpt-1b':
        return 'rinna/japanese-gpt-1b', './results/rinna-japanese-gpt-1b.gconcat', 100
    elif mode == 'xlm-roberta-large':
        return 'xlm-roberta-large', './results/xlm-roberta-large.gconcat', 100
    elif mode == 'xlm-roberta-base':
        return 'xlm-roberta-base', './results/xlm-roberta-base.gconcat', 100
...
def train_model(run_mode='rinna-gpt2', index_fold=0):
    ...
    OUTPUT_PATH = OUTPUT_PATH + f'.230418.{seed}'
    ...
    s1, y = [tokenizer(d['s1'] + f' {tokenizer.sep_token if tokenizer.sep_token != "" else "[SEP]"} ' + d['s2'], return_tensors='pt') for d in datasets['train']], [d['label'] for d in datasets['train']]
    train_dataset = Dataset(s1, y, datasets['train'])
    s1, y = [tokenizer(d['s1'] + f' {tokenizer.sep_token if tokenizer.sep_token != "" else "[SEP]"} ' + d['s2'], return_tensors='pt') for d in datasets['dev']], [d['label'] for d in datasets['dev']]
    dev_dataset = Dataset(s1, y, datasets['dev'])
    s1, y = [tokenizer(d['s1'] + f' {tokenizer.sep_token if tokenizer.sep_token != "" else "[SEP]"} ' + d['s2'], return_tensors='pt') for d in datasets['test']], [d['label'] for d in datasets['test']]
    ...
    for e in range(EPOCHS):
        ...
        with torch.inference_mode():
            ...
            for i, (s1, y , m1, allx) in enumerate(dev_dataloader):
                ...
                dev_predictions.extend([items[-1] | {'predicted_label': {v: k for k, v in label_indexer.items()}[pred], 'predicted_index': pred, 'logits': logit, 'last_hidden_state': hidden} for items, pred, logit, hidden in zip(allx, predicted_label.tolist(), logits.tolist(), h.tolist())])
            ...
            for i, (s1, y , m1, allx) in enumerate(test_dataloader):
                ...
                test_predictions.extend([items[-1] | {'predicted_label': {v: k for k, v in label_indexer.items()}[pred], 'predicted_index': pred, 'logits': logit, 'last_hidden_state': hidden} for items, pred, logit, hidden in zip(allx, predicted_label.tolist(), logits.tolist(), h.tolist())])
        ...
        if accelerator is None:
            ...
            with Path(f'{OUTPUT_PATH}/result.concat.{run_mode.replace("/", ".")}.fold{index_fold}.csv').open('w') as f:
...
if __name__ == '__main__':
    ...
    if is_single:
        train_model(run_modes[1], index_fold=0)
    else:
