...
class Classifier(nn.Module):
    ...
    def forward(self, x1, mask1, x2, mask2):
        ...
        if self.pooling_method == 'mean':
            ...
            x2 = self.pooling_mean(x2, mask2)
        elif self.pooling_method == 'max':
            x1 = self.pooling_max(x1, mask1)
            x2 = self.pooling_max(x2, mask2)
...
def get_properties(mode):
    if mode == 'rinna-gpt2':
        ...
        return 'nlp-waseda/roberta-base-japanese-with-auto-jumanpp', './results/nlp-waseda-roberta-base-japanese.concat', 100
    elif mode == 'nlp-waseda-roberta-large-japanese-with-auto-jumanpp':
        return 'nlp-waseda/roberta-large-japanese-with-auto-jumanpp', './results/nlp-waseda-roberta-large-japanese.concat', 100
...
def train_model(run_mode='rinna-gpt2', index_fold=0):
    ...
    OUTPUT_PATH = OUTPUT_PATH + f'.230227.{seed}'
    ...
    for e in range(EPOCHS):
        ...
        for i, (s1, s2, y , m1, m2, allx) in enumerate(train_dataloader):
            ...
            logits, _ = classifier(h1, m1, h2, m2)
        ...
        with torch.inference_mode():
            ...
            for i, (s1, s2, y , m1, m2, allx) in enumerate(dev_dataloader):
                ...
                logits, h = classifier(h1, m1, h2, m2)
                loss = loss_func(logits, y)
                ...
                dev_predictions.extend([items[3] | {'predicted_label': {v: k for k, v in label_indexer.items()}[pred], 'predicted_index': pred, 'logits': logit, 'last_hidden_state': hidden} for items, pred, logit, hidden in zip(allx, predicted_label.tolist(), logits.tolist(), h.tolist())])
            ...
            for i, (s1, s2, y , m1, m2, allx) in enumerate(test_dataloader):
                ...
                test_predictions.extend([items[3] | {'predicted_label': {v: k for k, v in label_indexer.items()}[pred], 'predicted_index': pred, 'logits': logit, 'last_hidden_state': hidden} for items, pred, logit, hidden in zip(allx, predicted_label.tolist(), logits.tolist(), h.tolist())])
