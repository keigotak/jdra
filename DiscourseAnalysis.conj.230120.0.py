...
def train_model(run_mode='rinna-gpt2'):
    ...
    OUTPUT_PATH = OUTPUT_PATH + '.230121.0'
    ...
    s1, y = [tokenizer(d['s1'] + token_conj + d['s2'], return_tensors='pt') for d in datasets['train']], [d['label'] for d in datasets['train']]
    train_dataset = Dataset(s1, y, datasets['train'])
    s1, y = [tokenizer(d['s1'] + token_conj + d['s2'], return_tensors='pt') for d in datasets['dev']], [d['label'] for d in datasets['dev']]
    dev_dataset = Dataset(s1, y, datasets['dev'])
    s1, y = [tokenizer(d['s1'] + token_conj + d['s2'], return_tensors='pt') for d in datasets['test']], [d['label'] for d in datasets['test']]
    ...
    for e in range(EPOCHS):
        ...
        if accelerator is None:
            ...
            with Path(f'{OUTPUT_PATH}/result.conj.{model_name.replace("/", ".")}.csv').open('w') as f:
