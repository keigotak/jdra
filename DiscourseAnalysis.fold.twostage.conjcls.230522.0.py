...
class Classifier(nn.Module):
    ...
    def forward(self, x_conj, x_cls):
        ...
        x_conj = self.dropout(x_conj)
        x_detail = self.linear_detail(x_conj)
        x_binary = self.linear_binary(x_cls)
...
def get_properties(mode):
    if mode == 'rinna-gpt2':
        return 'rinna/japanese-gpt2-medium', './results/rinna-japanese-gpt2-medium.twostage.conjcls', 100
    elif mode == 'tohoku-bert':
        return 'cl-tohoku/bert-base-japanese-whole-word-masking', './results/bert-base-japanese-whole-word-masking.twostage.conjcls', 100
    elif mode == 'mbart':
        return 'facebook/mbart-large-cc25', './results/mbart-large-cc25.twostage.conjcls', 100
    elif mode == 't5-base-encoder':
        return 'megagonlabs/t5-base-japanese-web', './results/t5-base-japanese-web.twostage.conjcls', 100
    elif mode == 't5-base-decoder':
        return 'megagonlabs/t5-base-japanese-web', './results/t5-base-japanese-web.twostage.conjcls', 100
    elif mode =='rinna-roberta':
        return 'rinna/japanese-roberta-base', './results/rinna-japanese-roberta-base.twostage.conjcls', 100
    elif mode == 'nlp-waseda-roberta-base-japanese':
        return 'nlp-waseda/roberta-base-japanese', './results/nlp-waseda-roberta-base-japanese.twostage.conjcls', 100
    elif mode == 'nlp-waseda-roberta-large-japanese':
        return 'nlp-waseda/roberta-large-japanese', './results/nlp-waseda-roberta-large-japanese.twostage.conjcls', 100
    elif mode == 'nlp-waseda-roberta-base-japanese-with-auto-jumanpp':
        return 'nlp-waseda/roberta-base-japanese-with-auto-jumanpp', './results/nlp-waseda-roberta-base-japanese.twostage.conjcls', 100
    elif mode == 'nlp-waseda-roberta-large-japanese-with-auto-jumanpp':
        return 'nlp-waseda/roberta-large-japanese-with-auto-jumanpp', './results/nlp-waseda-roberta-large-japanese.twostage.conjcls', 100
    elif mode == 'rinna-japanese-gpt-1b':
        return 'rinna/japanese-gpt-1b', './results/rinna-japanese-gpt-1b.twostage.conjcls', 100
    elif mode == 'xlm-roberta-large':
        return 'xlm-roberta-large', './results/xlm-roberta-large.twostage.conjcls', 100
    elif mode == 'xlm-roberta-base':
        return 'xlm-roberta-base', './results/xlm-roberta-base.twostage.conjcls', 100
...
def train_model(run_mode='rinna-gpt2', index_fold=0):
    ...
    for e in range(EPOCHS):
        ...
        for i, (s1, yb, yd, m1, allx) in enumerate(train_dataloader):
            ...
            h1_conj = torch.stack([h[item.index(id_conj)] for h, item in zip(h1, s1.tolist())])
            h1_cls = torch.stack([h[item.index(id_cls)] for h, item in zip(h1, s1.tolist())])
            outputs = classifier(h1_conj, h1_cls)
