...
import json
from Helperfunctions import get_metrics_scores, get_metrics_scores_by_divided_type, get_confusion_matrix2
...
def gather_embeddings():
    ...
    for mode in MODE_LIST:
        ...
        for iseed in range(num_seed):
            ...
            labels = ['原因・理由', '逆接・譲歩', '条件', '目的', '根拠', '対比']
...
def gather_confusion_matrics():
    ...
    num_fold = 5
    label_indexer = {k: i for i, k in enumerate(['原因・理由', '目的', '条件', '根拠', '対比', '逆接・譲歩', 'その他根拠', '談話関係なし'])}
    key_labels = ['原因・理由', '逆接', '条件', '目的', '根拠', '対比', '談話関係なし']
    ...
    label_modifier = {label_indexer[dict_label[label]]: i for i, label in enumerate(key_labels)}
    ...
    for mode in MODE_LIST:
        ...
        for iseed in range(num_seed):
            ...
            y_true = [label_modifier[r['label']] for r in dev_results]
            y_pred = [label_modifier[r['predicted_index']] for r in dev_results]
            dev_cm = get_confusion_matrix2(y_true=y_true, y_pred=y_pred, num_class=len(key_labels))
            # dev_cm = [[cell for j, cell in enumerate(row) if j != label_indexer['その他根拠']] for i, row in enumerate(dev_cm) if i != label_indexer['その他根拠']]

            y_true = [label_modifier[r['label']] for r in test_results]
            y_pred = [label_modifier[r['predicted_index']] for r in test_results]
            test_cm = get_confusion_matrix2(y_true=y_true, y_pred=y_pred, num_class=len(key_labels))
            # test_cm = [[cell for j, cell in enumerate(row) if j != label_indexer['その他根拠']] for i, row in enumerate(test_cm) if i != label_indexer['その他根拠']]

        key_labels_en = [dict_label_en[dict_label[label]] for label in key_labels]
        cm = pd.DataFrame(data=test_cm, index=key_labels_en, columns=key_labels_en)
        ax = sns.heatmap(cm, square=True, cbar=True, annot=True, cmap='Blues', fmt=',d')
