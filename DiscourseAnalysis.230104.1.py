...
class Classifier(nn.Module):
    def __init__(self, hidden_size, num_layers, num_heads, num_class):
        super(Classifier, self).__init__()
        self.encoder_f = nn.ModuleList([nn.GRU(hidden_size, hidden_size) for _ in range(num_layers)])
        self.encoder_b = nn.ModuleList([nn.GRU(hidden_size, hidden_size) for _ in range(num_layers)])
...
def train_model(run_mode='rinna-gpt2'):
    ...
    classifier = allocate_data_to_device(Classifier(model.config.hidden_size, NUM_LAYERS, NUM_HEADS, len(label_indexer)), DEVICE)

    optimizer = AdamW(params=list(model.parameters()) + list(classifier.parameters()), lr=2e-6 if 'xlm' in model_name else 2e-5, weight_decay=0.01)
    ...
    for e in range(100):
        ...
        for d in datasets['train']:
            ...
            logits = classifier(torch.cat([h1, h2], dim=1))
        ...
        if vw.is_updated():
            with Path(f'{OUTPUT_PATH}/{model_name.replace("/", ".")}.{e}.pt').open('wb') as f:
                torch.save({'model': model.state_dict(), 'classifier': classifier.state_dict()}, f)
