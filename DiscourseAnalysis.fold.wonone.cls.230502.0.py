...
def get_data(resource='expert', index_fold=0):
    ...
    all_datasets = {}
    label_indexer = {k: i for i, k in enumerate(['原因・理由', '目的', '条件', '根拠', '対比', '逆接・譲歩', 'その他根拠'])}
    for mode in modes:
        datasets = []
        _ids = set(ids[mode])
        for p in problems:
...
def get_data_230205(resource='expert', index_fold=0):
    ...
    label_indexer = {k: i for i, k in enumerate(['原因・理由', '目的', '条件', '根拠', '対比', '逆接', 'その他根拠'])}
    for mode in modes:
...
def get_properties(mode):
    if mode == 'rinna-gpt2':
        return 'rinna/japanese-gpt2-medium', './results/rinna-japanese-gpt2-medium.wonone.cls', 100
    elif mode == 'tohoku-bert':
        return 'cl-tohoku/bert-base-japanese-whole-word-masking', './results/bert-base-japanese-whole-word-masking.wonone.cls', 100
    elif mode == 'mbart':
        return 'facebook/mbart-large-cc25', './results/mbart-large-cc25.wonone.cls', 100
    elif mode == 't5-base-encoder':
        return 'megagonlabs/t5-base-japanese-web', './results/t5-base-japanese-web.wonone.cls', 100
    elif mode == 't5-base-decoder':
        return 'megagonlabs/t5-base-japanese-web', './results/t5-base-japanese-web.wonone.cls', 100
    elif mode =='rinna-roberta':
        return 'rinna/japanese-roberta-base', './results/rinna-japanese-roberta-base.wonone.cls', 100
    elif mode == 'nlp-waseda-roberta-base-japanese':
        return 'nlp-waseda/roberta-base-japanese', './results/nlp-waseda-roberta-base-japanese.wonone.cls', 100
    elif mode == 'nlp-waseda-roberta-large-japanese':
        return 'nlp-waseda/roberta-large-japanese', './results/nlp-waseda-roberta-large-japanese.wonone.cls', 100
    elif mode == 'nlp-waseda-roberta-base-japanese-with-auto-jumanpp':
        return 'nlp-waseda/roberta-base-japanese-with-auto-jumanpp', './results/nlp-waseda-roberta-base-japanese.wonone.cls', 100
    elif mode == 'nlp-waseda-roberta-large-japanese-with-auto-jumanpp':
        return 'nlp-waseda/roberta-large-japanese-with-auto-jumanpp', './results/nlp-waseda-roberta-large-japanese.wonone.cls', 100
    elif mode == 'rinna-japanese-gpt-1b':
        return 'rinna/japanese-gpt-1b', './results/rinna-japanese-gpt-1b.wonone.cls', 100
    elif mode == 'xlm-roberta-large':
        return 'xlm-roberta-large', './results/xlm-roberta-large.wonone.cls', 100
    elif mode == 'xlm-roberta-base':
        return 'xlm-roberta-base', './results/xlm-roberta-base.wonone.cls', 100
...
def train_model(run_mode='rinna-gpt2', index_fold=0):
    ...
    OUTPUT_PATH = OUTPUT_PATH + f'.230502.{seed}'
    ...
    s1, y = [tokenizer(d['s1'], text_pair=d['s2'], return_tensors='pt') for d in datasets['train']], [d['label'] for d in datasets['train']]
    train_dataset = Dataset(s1, y, datasets['train'])
    s1, y = [tokenizer(d['s1'], text_pair=d['s2'], return_tensors='pt') for d in datasets['dev']], [d['label'] for d in datasets['dev']]
    dev_dataset = Dataset(s1, y, datasets['dev'])
    s1, y = [tokenizer(d['s1'], text_pair=d['s2'], return_tensors='pt') for d in datasets['test']], [d['label'] for d in datasets['test']]
    ...
    for e in range(EPOCHS):
        ...
        if accelerator is None:
            ...
            with Path(f'{OUTPUT_PATH}/result.cls.{run_mode.replace("/", ".")}.fold{index_fold}.csv').open('w') as f:
