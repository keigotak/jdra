...
def tokenize_sentences(dataset, tokenizer):
    ...
    for items in dataset:
        tokens1 = tokenizer(items[1], text_pair=items[2], return_tensors='pt')
        _dataset.append(items + [tokens1])
    return _dataset
...
def train_model(run_mode='rinna-gpt2'):
    ...
    output_layer = allocate_data_to_device(torch.nn.Linear(model.config.hidden_size, len(label_indexer)), DEVICE)
