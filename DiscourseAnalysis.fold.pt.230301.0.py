...
def get_properties(mode):
    if mode == 'rinna-gpt2':
        return 'rinna/japanese-gpt2-medium', './results/rinna-japanese-gpt2-medium.pt', 100
    elif mode == 'tohoku-bert':
        return 'cl-tohoku/bert-base-japanese-whole-word-masking', './results/bert-base-japanese-whole-word-masking.pt', 100
    elif mode == 'mbart':
        return 'facebook/mbart-large-cc25', './results/mbart-large-cc25.pt', 100
    elif mode == 't5-base-encoder':
        return 'megagonlabs/t5-base-japanese-web', './results/t5-base-japanese-web.pt', 100
    elif mode == 't5-base-decoder':
        return 'megagonlabs/t5-base-japanese-web', './results/t5-base-japanese-web.pt', 100
    elif mode =='rinna-roberta':
        return 'rinna/japanese-roberta-base', './results/rinna-japanese-roberta-base.pt', 100
    elif mode == 'nlp-waseda-roberta-base-japanese':
        return 'nlp-waseda/roberta-base-japanese', './results/nlp-waseda-roberta-base-japanese.pt', 100
    elif mode == 'nlp-waseda-roberta-large-japanese':
        return 'nlp-waseda/roberta-large-japanese', './results/nlp-waseda-roberta-large-japanese.pt', 100
    elif mode == 'nlp-waseda-roberta-base-japanese-with-auto-jumanpp':
        return 'nlp-waseda/roberta-base-japanese-with-auto-jumanpp', './results/nlp-waseda-roberta-base-japanese.pt', 100
    elif mode == 'nlp-waseda-roberta-large-japanese-with-auto-jumanpp':
        return 'nlp-waseda/roberta-large-japanese-with-auto-jumanpp', './results/nlp-waseda-roberta-large-japanese.pt', 100
    elif mode == 'rinna-japanese-gpt-1b':
        return 'rinna/japanese-gpt-1b', './results/rinna-japanese-gpt-1b.pt', 100
    elif mode == 'xlm-roberta-large':
        return 'xlm-roberta-large', './results/xlm-roberta-large.pt', 100
    elif mode == 'xlm-roberta-base':
        return 'xlm-roberta-base', './results/xlm-roberta-base.pt', 100
...
def train_model(run_mode='rinna-gpt2', index_fold=0):
    ...
    OUTPUT_PATH = OUTPUT_PATH + f'.230301.{seed}'
    ...
    if with_load_weight:
        model = AutoModel.from_pretrained('./results/xlm-roberta-large.pt.230301')
    ...
    s1, y = [tokenizer(f"{header}\n\n文１：{d['s1']}\n文２：{d['s2']}\n\n答え：{token_ans}", return_tensors='pt') for d in datasets['train']], [d['label'] for d in datasets['train']]
    train_dataset = Dataset(s1, y, datasets['train'])
    s1, y = [tokenizer(f"{header}\n\n文１：{d['s1']}\n文２：{d['s2']}\n\n答え：{token_ans}", return_tensors='pt') for d in datasets['dev']], [d['label'] for d in datasets['dev']]
    dev_dataset = Dataset(s1, y, datasets['dev'])
    s1, y = [tokenizer(f"{header}\n\n文１：{d['s1']}\n文２：{d['s2']}\n\n答え：{token_ans}", return_tensors='pt') for d in datasets['test']], [d['label'] for d in datasets['test']]
    ...
    for e in range(EPOCHS):
        ...
        for i, (s1, y , m1, allx) in enumerate(train_dataloader):
            ...
            h1 = torch.stack([h[item.index(id_ans)] for h, item in zip(h1, s1.tolist())])
